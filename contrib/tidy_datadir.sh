...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Worldleadcurrency database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Worldleadcurrency datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Worldleadcurrency datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Worldleadcurrency 0.7 datadir."
    ;;
  3)
    echo "Detected Worldleadcurrency pre-0.8 datadir."
    ;;
  4)
    echo "Detected Worldleadcurrency 0.8 datadir."
    ;;
esac
