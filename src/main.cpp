...
// solidar will accept the old and the new ChainID
int GetOldChainID()
{
    return 0x0011;
}
...
bool CBlockHeader::CheckProofOfWork(int nHeight) const
{
    if (nHeight >= GetAuxPowStartBlock())
    {
        ...
        if (!fTestNet && nHeight != INT_MAX && !(GetChainID() != GetOurChainID() || GetChainID() != GetOldChainID()))
...
unsigned char pchMessageStart[4] = { 0x5b, 0x6c, 0x44, 0xa4 };
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey)
{
	...
	mpq rounding = mpq("1/1");
	mpq nBlockReward = (GetInitialDistributionAmount(nHeight) + GetPerpetualSubsidyAmount(nHeight)) * (1 - getTitheRatio(nHeight)) + nFees - rounding;
