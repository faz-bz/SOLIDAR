...
#else
    const char* pszModule = "worldleadcurrency";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Worldleadcurrency";
#else
    fs::path pathRet;
    ...
    return pathRet / "Worldleadcurrency";
#else
    // Unix
    return pathRet / ".worldleadcurrency";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "worldleadcurrency.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "worldleadcurrencyd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Worldleadcurrency will not work properly.");
